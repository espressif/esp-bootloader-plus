...
#define BOOTLOADER_STORAGE_FLASH_STREAM_SUPPORT CONFIG_BOOTLOADER_DIFF_ENABLED
...
/**
 * When reading flash in bootlaoder, src_addr, dest_addr and read_size have to be 4-byte aligned.
...
esp_err_t bootloader_storage_flash_read(size_t src_addr, void *dest, size_t size, bool allow_decrypt)
{
    ...
    if (bootloader_storage_flash_stream_read(src_addr, size, allow_decrypt) == 0) {
        ESP_LOGE(TAG, "read to stream buf err");
        return ESP_FAIL;
    }
    memcpy(dest, byte_stream->stream_buffer + (src_addr - byte_stream->left_addr), size);
